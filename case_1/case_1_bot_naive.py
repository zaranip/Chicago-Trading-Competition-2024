...
class MainBot(xchange_client.XChangeClient):
    ...
    async def trade(self):
        ...
        while True:
            ...
            bids = dict((symbol, self.get_last_transacted_price(symbol, xchange_client.Side.BUY) - 1) for symbol in SYMBOLS + ETFS)
            asks = dict((symbol, self.get_last_transacted_price(symbol, xchange_client.Side.SELL) + 1) for symbol in SYMBOLS + ETFS)

            for symbol in SYMBOLS + ETFS:
                bids[symbol] = max(self.order_books[symbol].bids) if len(self.order_books[symbol].bids) > 0 else bids[symbol]
                asks[symbol] = min(self.order_books[symbol].asks) if len(self.order_books[symbol].asks) > 0 else asks[symbol]
