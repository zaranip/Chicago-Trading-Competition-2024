...
class OpenOrders:
    def __init__(self):
        ...
        self.trap_ids = dict((symbol, {"ASK": 0, "BID": 0}) for symbol in SYMBOLS + ETFS + ASSET)
...
class MainBot(xchange_client.XChangeClient):
    ...
    async def trade(self):
        ...
        # TODO: make meaningful mutex
        mutex = threading.Semaphore(1)

        threads = [None for symbol in SYMBOLS + ETFS + ASSET]
        for i, symbol in enumerate(SYMBOLS + ETFS + ASSET):
            threads[i] = threading.Thread(target=self.logic, args=(symbol))
            threads[i].start()
