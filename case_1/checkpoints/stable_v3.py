...
class MainBot(xchange_client.XChangeClient):
    ...
    def __init__(self, host: str, username: str, password: str, open_orders):
        ...
        self.last_transacted_price = dict((symbol, {side: {
            "price": 0, "age": 0
        } for side in [xchange_client.Side.BUY, xchange_client.Side.SELL]}) for symbol in SYMBOLS + ETFS)
    ...
    async def bot_place_arbitrage_order(self, etf, side):
        ...
        qty = 1 
        if side == "from":
            await self.bot_place_order(etf, qty * 10, xchange_client.Side.BUY, market=True)
        elif side == "to":
            for symbol in convert[etf]:
                await self.bot_place_order(symbol, qty * 10, xchange_client.Side.BUY, market=True)
        ...
        if side == "from":
            for symbol in convert[etf]:
                await self.bot_place_order(symbol, qty * 10, xchange_client.Side.SELL, 0, market=True)
        elif side == "to":
            await self.bot_place_order(etf, qty *10, xchange_client.Side.SELL, market=True)
    ...
    async def trade(self):
        ...
        while True:
            ...
            for etf in ETFS:
                fair = dict((symbol, np.mean(self.history[symbol][-25:]) if len(self.history[symbol]) >= 25 else np.mean(self.history[symbol])) for symbol in SYMBOLS + ETFS)
                if etf == "SCP":
