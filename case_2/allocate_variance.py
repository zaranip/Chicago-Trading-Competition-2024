...
def grading(train_data, test_data, window_size):
    num_windows = len(test_data) - window_size + 1
    sharpe_ratios = []
    capitals = []
    weights_list = []

    for i in range(num_windows):
        window_train_data = train_data.iloc[i:i+window_size]
        window_test_data = test_data.iloc[i:i+window_size]
...
sharpe_ratios, capitals, weights_list = grading(TRAIN, TEST, window_size)
