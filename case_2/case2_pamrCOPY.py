...
class Allocator():
    def __init__(self, train_data, eps=0.5, C=500, variant=0, max_weight=0.3, window=10, trend_factor=0.1, noise_factor=0.1):
    ...
    def allocate_portfolio(self, asset_prices):
        ...
        mavg = self.running_price_paths.rolling(window=self.window).mean().iloc[-1]

        # Adjust weights based on market trends
        trends = asset_prices / mavg
        self.last_b = self.update(self.last_b, asset_prices / asset_prices.mean(), self.eps, self.C)
        self.last_b = self.last_b * (1 + self.trend_factor * (trends - 1))

        # Add random noise to weights to encourage fluctuations
        noise = np.random.normal(0, self.noise_factor, self.last_b.shape)
        self.last_b = np.abs(self.last_b + noise)

        # Normalize weights
        self.last_b /= np.sum(self.last_b)
